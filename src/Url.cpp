...
#undef base_ctors
  ...
  void Url::assign(const string& s)
  {
    ...
    try {
      ...
      string s("Url::assign() std::regex_error: ");
      s+=re.what();
...
/***** ACCESSORS *****/
  void Url::scheme(const string& s)
  {
    try {
      ...
      throw UrlParseError("Url::scheme("+s+"): std::regex_error: " + re.what());
    } catch(UrlParseError e) {
      throw;
    } catch(...) {
 ...
 */
  void Url::authority(const string& s)
  {
    try {
      ...
      throw UrlParseError("Url::authority("+s+"): std::regex_error: " + re.what());
    } catch(UrlParseError e) {
      throw;
    } catch(...) {
  ...
  void Url::host(const string& s)
  {
    ...
    try {
      ...
      throw UrlParseError("Url::host("+s+"): std::regex_error: " + re.what());
    } catch(...) {
  ...
  void Url::port(const string& s)
  {
    ...
    try {
      ...
      throw UrlParseError("Url::port("+s+"): std::regex_error: " + re.what());
    } catch(UrlParseError e) {
      throw;
    } catch(...) {
